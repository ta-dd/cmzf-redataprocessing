...
def getting_offers_without_downloaded_description(
    ...
    if c.fetchone()[0] == 1:
        ...
        indices = pd.read_sql(
            "SELECT hash_id FROM {} WHERE hash_id not in (SELECT hash_id FROM {})".format(
                db_table_name_offers, db_table_name_description
            ),
            con=con,
        )
    else:
        ...
        indices = pd.read_sql(
            "SELECT hash_id FROM {}".format(db_table_name_offers), con=con
        )
    c.close()
    con.close()
...
def note_missing_values(r_dict_names_all: Dict) -> None:
    ...
    if len(r_dict_names_all[~r_dict_names_all.isin(description_items_dict.keys())]) > 0:
        print(
            "Add these values to description_items_dict in sreality_api_dictionaries.py:"
        )
...
def description_decoding(responses_list: list) -> pd.DataFrame:
    ...
    with tqdm(
        total=total, desc="decoding responses with description of offers: "
    ) as pbar:
        for r_dict in responses_list:
            try:
                ...
                r_dict_names_all = pd.concat(
                    [
                        r_dict_names_all,
                        r_dict_names[~r_dict_names.isin(r_dict_names_all)],
                    ]
                )

                r_dict_types = r_dict_values[["type", "name"]]
                r_dict_types_all = pd.concat(
                    [
                        r_dict_types_all,
                        r_dict_types.loc[
                            ~r_dict_types["name"].isin(r_dict_types_all["name"]), :
                        ],
                    ]
                )
...
def get_re_offers_description(
    ...
    indices = getting_offers_without_downloaded_description(
    ...
    db_table_name_description = "DESCRIPTION_" + db_table_name
    print(
        "saving description of offers to database (table {})".format(
            db_table_name_description
        )
    )
    ...
    print(
        "saved description of offers to database (table {})".format(
            db_table_name_description
        )
    )
