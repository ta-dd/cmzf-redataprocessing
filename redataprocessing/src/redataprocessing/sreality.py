...
def download_lists(
    category_main: int,
    category_type: int,
    locality_region_id: Optional[Union[int, list]] = None,
    category_sub: Optional[Union[int, list]] = None,
) -> list:
    """
...
def save_re_offers(
    df: pd.DataFrame, path_to_sqlite: str, category_main: int, category_type: int
) -> None:
    """
    ...
    db_table_name = create_db_table_name(
    ...
    if c.fetchone()[0] == 1:
        ...
        for i in df.columns:
            if i not in list_of_colnames:
                c.execute(
                    "ALTER TABLE {} ADD {} VARCHAR(100);".format(
                        db_table_name_offers, i
                    )
                )
...
def get_re_offers(
    ...
    category_main: str,
    category_type: str,
    locality_region: Optional[Union[str, list]],
    category_sub: Optional[Union[str, list]] = None,
) -> None:
    """
    ...
    if isinstance(category_main, str):
        if category_main not in category_main_dict.keys():
            raise ValueError(
                "category_main: must be one of %r." % list(category_main_dict.keys())
            )
        else:
    ...
    if isinstance(category_type, str):
        if category_type not in category_type_dict.keys():
            raise ValueError(
                "category_type: must be one of %r." % list(category_type_dict.keys())
            )
        else:
    ...
    if isinstance(category_sub, str):
        if category_sub not in category_sub_dict.keys():
            raise ValueError(
                "locality_region: must be one or more of %r."
                % list(category_sub_dict.keys())
            )
        ...
        for idx in category_sub:
            if idx not in category_sub_dict.keys():
                raise ValueError(
                    "category_sub: must be one or more of %r."
                    % list(category_sub_dict.keys())
                )
    ...
    if isinstance(locality_region, str):
        if locality_region not in locality_region_id_dict.keys():
            raise ValueError(
                "locality_region: must be one or more of %r."
                % list(locality_region_id_dict.keys())
            )
        ...
        for idx in locality_region:
            if idx not in locality_region_id_dict.keys():
                raise ValueError(
                    "locality_region: must be one of %r."
                    % list(locality_region_id_dict.keys())
                )
            locality_region_id_input.append(locality_region_id_dict[idx])
    elif len(locality_region) > 1:
        raise ValueError(
            "locality_region: this package version is not able to handle more than one value"
        )
    else:
    ...
    if isinstance(path_to_sqlite, str):
        if os.path.dirname(path_to_sqlite) == "":
            print(
                "resulting data will be saved to current working directory ({})".format(
                    os.getcwd()
                )
            )
            ...
            FileNotFoundError(
                "{} folder does not exist!".format(os.path.dirname(path_to_sqlite))
            )
    else:
